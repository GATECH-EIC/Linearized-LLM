...
def generate_candidates(medusa_logits, logits, tree_indices, retrieve_indices):
    ...
    #print(candidates_logit.device, candidates_medusa_logits.device)
    # Combine the selected candidate from the original logits with the topk medusa logits.
    candidates = torch.cat([candidates_logit, candidates_medusa_logits.view(-1).to(candidates_logit.device)], dim=-1)

    # Map the combined candidates to the tree indices to get tree candidates.
    tree_candidates = candidates[tree_indices.to(candidates.device)]
    ...
    cart_candidates = tree_candidates_ext[retrieve_indices.to(tree_candidates_ext.device)]
...
def tree_decoding(
    ...
    medusa_logits = tree_medusa_logits[:, 0, retrieve_indices.to(tree_medusa_logits.device)]
