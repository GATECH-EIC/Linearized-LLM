...
GROUP_EXACT = int(os.environ.get("GROUP_SIZE", 0))

medusa_status = [0, None]
...
def _local_quadratic_attn_p_g1( q, k, v, causal_mask=None):
    qk = torch.einsum('bhns,bhms->bhnm', q, k.to(q.device))

    a = torch.nn.functional.relu(qk) ** 2
    if causal_mask is not None:
        a = causal_mask.unsqueeze(1).to(q.device) * a
    #a = causal_mask(a) if causal else a
    return torch.einsum('bhnm,bhme->bhne', a, v.to(q.device))

def _local_quadratic_attn_p_g1_mt( q, k, v,GROUP_SIZE, causal_mask=None, mt=None):
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        if USE_LINEAR:
            #print("size: ", query_states.size(), key_states.size(), value_states.size())
            import einops
            ...
            if medusa_status[0] == 0: #0 for prefill stage
                #print("prefill")
                if not res and not res_k:
                    ...
                    local_out0 = _local_quadratic_attn_p(query_t, key_t.to(query_t.device), value_t.to(query_t.device), causal_mask=attention_mask0.to(query_t.device))
                    ...
                    assert query_t_pre.size(2) == key_t_pre.size(2)
                    if query_t_pre.size(2) > 0:
                        local_out_pre = _local_quadratic_attn_p(query_t_pre, key_t_pre.to(query_t_pre.device), value_t_pre.to(query_t_pre.device), 
                        causal_mask=attention_mask_pre.to(query_t_pre.device))
                        local_out_pre = einops.rearrange(local_out_pre, "b h g c d -> b h (g c) d")

                    local_out_res = _local_quadratic_attn_p_g1(query_t_res, key_t_res.to(query_t_res.device), value_t_res.to(query_t_res.device), 
                    causal_mask=attention_mask_res.to(query_t_res.device))
                    if query_t_pre.size(2) > 0:
                ...
                if USE_GLOBAL:                    
                    if not res:
                        global_out0 = _global_linear_attn_p(query_t, key_t.to(query_t.device), value_t.to(query_t.device), causal=True)
                        global_out = einops.rearrange(global_out0, "b h g c d -> b h (g c) d")
                    else:
                        global_out0 = _global_linear_attn_causal(query_t_pre, key_t_pre.to(query_t_pre.device), value_t_pre.to(query_t_pre.device), 
                        query_t_res.to(query_t_pre.device), key_t_res.to(query_t_pre.device), value_t_res.to(query_t_pre.device))
            ...
            elif medusa_status[0] == 1: #speculative
                guess_len = query_t.size(2)
                k_len = key_t.size(2)
                res_k_wo_guess = (k_len - guess_len) % GROUP_SIZE
                attention_mask_res = attention_mask0[:, - res_k_wo_guess - guess_len:]
                attention_mask_res = attention_mask_res.unsqueeze(0)


                maintain = int(os.environ.get("maintain", 1))
                query_t_res = query_t

                key_t_res = key_t[:,:,-guess_len-res_k_wo_guess:,:]

                value_t_res = value_t[:,:,-guess_len-res_k_wo_guess:,:]
                
                offset = medusa_status[1].tolist()
                get_idx = [offset.index(idx) for idx in range(0, offset[-1] + 1)] #+ [len(offset)]
                #print("local: ", query_t_res.size(), key_t_res.size(), attention_mask_res.size(), offset, get_idx, res_k_wo_guess + offset[-1] + 1)
                assert key_t_res.size(2) == res_k_wo_guess + guess_len
                if maintain and res_k_wo_guess + offset[-1] + 1 > GROUP_SIZE:
                    first_group = get_idx[GROUP_SIZE - res_k_wo_guess]
                    #print("first: ", res_k_wo_guess, first_group, GROUP_SIZE - res_k_wo_guess, get_idx)
                    local_out_res0 = _local_quadratic_attn_p_g1(query_t_res[:,:,:first_group], key_t_res[:,:,:res_k_wo_guess+first_group], 
                    value_t_res[:,:,:res_k_wo_guess+first_group], causal_mask=attention_mask_res[:,:first_group,:res_k_wo_guess+first_group])                
                    local_out_res1 = _local_quadratic_attn_p_g1(query_t_res[:,:,first_group:], key_t_res[:,:,res_k_wo_guess+first_group:], 
                    value_t_res[:,:,res_k_wo_guess+first_group:], causal_mask=attention_mask_res[:,first_group:,res_k_wo_guess+first_group:])                
                    local_out_res = torch.cat((local_out_res0, local_out_res1), dim=2)
                else:
                ...
                if USE_GLOBAL:    
                    res_k = guess_len + res_k_wo_guess
                    #key_t_pre = key_t[:,:,:-guess_len-res_k_wo_guess,:] 
                    #value_t_pre = value_t[:,:,:-guess_len-res_k_wo_guess,:]
                    ...
                    kv = torch.einsum('bhgcs,bhgce->bhgse', key_t_pre, value_t_pre)
                    kv1 = torch.cumsum(kv, dim=2) 
                    if kv1.size(2) > 0: 
                        ##############not here
                        #we choose not to follow the original global

                        qkv_res = torch.einsum('bhcs,bhse->bhce', query_t_res, kv1[:,:,-1,:,:].to(query_t_res.device))
                        #qkv0 = torch.zeros((query_t_res.size()[0], query_t_res.size()[1], 0, query_t_res.size()[3], kv1.size()[-1]), device=query_t_res.device, dtype=query_t_res.dtype)
                    else:
                        qkv_res = torch.zeros((query_t_res.size()[0], query_t_res.size()[1], query_t_res.size()[2], kv1.size()[-1]), device=query_t_res.device, dtype=query_t_res.dtype) #torch.einsum('bhcs,bhse->bhce', q_res, kv1[:,:,0,:,:])
                        #qkv0 = torch.einsum('bhgcs,bhgse->bhgce', query_t_pre, kv1 - kv)
                ...
                if ADD_CONV:
                    self.dwconv.weight.data *= self.conv_mask
                    def build_medusa_conv(attn, mask):
                        medusa_len = mask.size(0)
                        mp = mask[:,-medusa_len:]
                        mm = torch.zeros(attn.num_heads, medusa_len + 2 * attn.dwconv.padding[0], \
                            medusa_len + 2 * attn.dwconv.padding[0], device=attn.dwconv.weight.device, dtype=attn.dwconv.weight.dtype)
                        c = attn.dwconv
                        conv_pos = attn.conv_pos.tolist()
                        for a, i in enumerate(range(c.padding[0], c.padding[0] + medusa_len)):
                            rel_pos = conv_pos[a]
                            past = mp[a].nonzero(as_tuple=True)[0].tolist() #the tokens this one can see
                            mm[:,i,rel_pos:c.padding[0]] = c.weight.data[:,0,:c.padding[0]-rel_pos,0]
                            for p, pa in enumerate(past):
                                mm[:,i,c.padding[0] + pa] = c.weight.data[:,0,c.padding[0] + p-rel_pos,0]
                        mreal = mm[:,c.padding[0]:-c.padding[0],:-c.padding[0]].unsqueeze(0)
                        
                        return mreal
                    if not hasattr(self, "medusa_conv"):
                        self.conv_pos = medusa_status[1].clone().detach()
                        self.medusa_conv  = build_medusa_conv(self, attention_mask0)
                    else:
                        if not((self.conv_pos == medusa_status[1]).all()):
                            self.conv_pos = medusa_status[1].clone().detach()
                            self.medusa_conv = build_medusa_conv(self, attention_mask0)

                    conv_out = self.medusa_conv.to(value_states.device).matmul(value_states[:,:,-self.dwconv.padding[0]-query_states.size(-2):,:])
                    #conv_out = self.dwconv(value_states)[:,:,-attn_output.size(2):,:] 
                    attn_output += conv_out.to(attn_output.device)

            elif medusa_status[0] == 2: #inference
                ...
                if not res:
                    ...
                    if query_t_pre.size(2) > 0:
                        local_out_pre = _local_quadratic_attn_p(query_t_pre, key_t_pre.to(query_t_pre.device), value_t_pre.to(query_t_pre.device), causal_mask=attention_mask_pre.to(query_t_pre.device))
                        local_out_pre = einops.rearrange(local_out_pre, "b h g c d -> b h (g c) d")
                    #print("size: ", query_t_pre.size(), query_t_res.size(), key_t_pre.size(), key_t_res.size())
                    local_out_res = _local_quadratic_attn_p_g1(query_t_res, key_t_res.to(query_t_pre.device), value_t_res.to(query_t_pre.device), causal_mask=attention_mask_res.to(query_t_pre.device))
                    if query_t_pre.size(2) > 0:
                        attn_output = torch.cat([local_out_pre, local_out_res], dim=2)
                    else:
                        attn_output = local_out_res
                ...
                if USE_GLOBAL:                    
                    if not res:
                        global_out0 = _global_linear_attn_p(query_t, key_t.to(query_t_pre.device), value_t.to(query_t_pre.device), causal=True)
                        global_out = einops.rearrange(global_out0, "b h g c d -> b h (g c) d")
                    else:
                        global_out0 = _global_linear_attn_causal(query_t_pre, key_t_pre.to(query_t_pre.device), value_t_pre.to(query_t_pre.device), 
                        query_t_res.to(query_t_pre.device), key_t_res.to(query_t_pre.device), value_t_res.to(query_t_pre.device))
